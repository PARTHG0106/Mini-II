...
class WorkoutSchedule(db.Model):
    ...
    user = db.relationship('User', backref=db.backref('workout_schedules', lazy=True))
    exercise = db.relationship('Exercises', backref=db.backref('workout_schedules', lazy=True))
...
class ScheduleCompletion(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    schedule_id = db.Column(db.Integer, db.ForeignKey('workout_schedule.id'), nullable=False)
    completed_at = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)
    
    schedule = db.relationship('WorkoutSchedule', backref=db.backref('completions', lazy=True))

    def __repr__(self):
        return f'<ScheduleCompletion {self.id} for schedule {self.schedule_id}>'
