...
def analyze_shoulder_press_video(video_path):
    """
    Analyzes shoulder press video and provides feedback on form
    """
    ...
    video_filename = f"shoulder_press_analysis_{timestamp}.mp4"
    ...
    in_press = False  # Track if currently in press position
    current_rep_feedback = [] # Feedback generated during the current rep
    
    # Rep detection thresholds
    MIN_ELBOW_ANGLE = 35  # Adjusted to match observed range
    MAX_ELBOW_ANGLE = 170  # Adjusted to match observed range
    
    # Thresholds for specific feedback
    SHOULDER_ANGLE_THRESH = 70  # Increased to allow more shoulder movement
    ELBOW_ANGLE_THRESH = 80  # Increased to allow more elbow movement
    ...
    with mp_pose.Pose(
        ...
        while True:
            ...
            shoulder_angle = None  # Initialize shoulder_angle to avoid UnboundLocalError
            ...
            if results.pose_landmarks:
                ...
                # Rep detection logic
                if not in_press and elbow_angle < MIN_ELBOW_ANGLE:
                    # Just entered press position
                    in_press = True
                    current_rep_feedback = [] # Clear feedback for the new rep
                    
                elif in_press:
                    # Check form while in press
                    
                    # 1. Check for IMPROPER SHOULDER POSITION
                    ...
                    # 2. Check for IMPROPER ELBOW POSITION
                    if elbow_angle < ELBOW_ANGLE_THRESH:
                        if "IMPROPER ELBOW POSITION" not in current_rep_feedback:
                            current_rep_feedback.append("IMPROPER ELBOW POSITION")
                            all_feedback.append("IMPROPER ELBOW POSITION")
                    
                    # Check if fully extended
                    if elbow_angle > MAX_ELBOW_ANGLE:
                        # Just completed press - Rep completed
                        in_press = False
            ...
            if active_feedback:
                ...
                # Add feedback text with line wrapping
                for i, feedback_text in enumerate(active_feedback):
                    y_pos = text_y_start + i * 35
                    wrapped_text = feedback_text.split(': ')
                    for j, line in enumerate(wrapped_text):
                        cv2.putText(display_frame, line, (20, y_pos + 20 + j * 20),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2, cv2.LINE_AA)
            ...
            if current_elbow_angle is not None:
                print(f"Elbow angle: {current_elbow_angle:.2f} degrees")
            if in_press:
                print("In press position")
            if rep_just_completed:
                print(f"Rep just completed. Correct reps: {correct_rep_count}, Incorrect reps: {incorrect_rep_count}")
            if current_rep_feedback:
                print(f"Current rep feedback: {current_rep_feedback}")
            if shoulder_angle > SHOULDER_ANGLE_THRESH:
                print(f"Shoulder angle {shoulder_angle:.2f} exceeds threshold {SHOULDER_ANGLE_THRESH}")
            if elbow_angle < ELBOW_ANGLE_THRESH:
                print(f"Elbow angle {elbow_angle:.2f} below threshold {ELBOW_ANGLE_THRESH}")
